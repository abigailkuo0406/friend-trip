...
export default function Navbar() {
  ...
  const [searchBarComponent, setSearchBarComponent] = useState(
    <Fragment></Fragment>
  )
  ...
  useEffect(() => {
    ...
    switch (router.pathname) {
      case '/product':
        setSearchBarComponent(
          <Fragment>
            <InputSearchBar
              localstorage="searchProduct"
              setGetSearch={setGetProductSearch}
        ...
        <div className="container position-relative d-flex align-items-center justify-content-between">
          ...
          {notifyModal && (
            <div className="position-absolute top-100 end-0">
              <UnacceptFriend unaccepts={unaccepts.rows2} items={setItem} />
            </div>
          )}
          ...
          <div className="d-flex align-items-center justify-content-between">
            {unaccepts.totalUnaccepts > 0 ? (
              <button
                type="button"
                className="position-relative btndefault me-3"
                onClick={openNotify}
              >
                <VscBell className="fs-4 bell" />
                <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bellHint">
                  {unaccepts.totalUnaccepts}
              ...
              <button
                type="button"
                class="position-relative btndefault me-3"
                onClick={openNotify}
              >
                <VscBell className="fs-4 bell" />
              </button>
