...
export default function Chatroom() {
  ...
  const today = moment().format('YYYY 年 MM 月 DD 日')
    ...
    <>
      ...
      <div className="container d-flex justify-content-center">
        <div className={`p-3 ${styles.dialogue} scrollbar-A`}>
          ...
          {/* <p>{chatText}</p> */}
          {chatTexts.map((c, i) => {
            return c.userName == auth.member_name ? (
              <div
                key={i}
                className={`ms-3 py-2 mb-2 d-flex justify-content-end align-items-center`}
              >
                <p className={`restLabel text-center ${styles.time} me-2`}>
                  {currentTime}
                </p>
                {/* <p className="restLabel me-4">{c.userName}</p> */}
                <p
                  className={`restLabel me-2 px-4 py-1 rounded ${styles.text}`}
                >
                  {c.message}
                </p>
              </div>
              ...
              <div
                key={i}
                className={`ms-3 py-2 mb-2 d-flex align-items-center`}
              >
                <Image
                ...
                <p className={`restLabel text-center ${styles.time}`}>
                  {currentTime}
                </p>
              </div>
            )
